...
chop. (For example, one solution might be the traditional iterative approach, one might be recursive,
one might use a functional style passing array slices around, and so on).

Goals
This Kata has three separate goals:

As you’re coding each algorithm, keep a note of the kinds of error you encounter. A binary search is
a ripe breeding ground for “off by one” and fencepost errors. As you progress through the week, see
if the frequency of these errors decreases (that is, do you learn from experience in one technique
when it comes to coding with a different technique?).

What can you say about the relative merits of the various techniques you’ve chosen? Which is the
...
It’s fairly hard to come up with five unique approaches to a binary chop. How did you go about
...
and memory performance are not issues (assuming the chop terminates before you get bored and kill it,
...
import codeKata2BinaryChop.CodeKata2BinaryChopSolution;
import org.junit.Test;

public class CodeKata2BinaryChopSolutionTest {

  CodeKata2BinaryChopSolution calc = new CodeKata2BinaryChopSolution();
  int[] emptyArray;

  @Test
  public void givenInt3AndAnEmptyArrayReturnMinus1(){
    assertEquals(-1, calc.chop(3,emptyArray));
  }
